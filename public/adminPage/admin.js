...
if (deleteModal) {
  deleteModal.addEventListener('show.bs.modal', event => {
    ...
    const id = button.getAttribute('data-bs-order');
...
document.getElementById('searchInput').addEventListener('input', () => {
  ...
  if (filteredOrders.length === 0) {
      ...
      Promise.all(filteredOrders.map(order => primeCard(order))).then(ordersCards => {
          root.innerHTML = ordersCards.join('');
      });
  }
});
    ...
    <div class="card shadow-sm mb-3 col-12" id="order-${order.orderId}">
      <div class="card-body d-flex justify-content-between align-items-center" dir="rtl">
        ...
        <div class="d-flex">
          ...
          <button type="button" class="btn btn-secondary me-1 ms-2" id="toggleButton${order.orderId}" onclick="showDetails('${order.orderId}')" data-bs-toggle="tooltip" data-bs-placement="top" title="הצג / הסתר פרטים">
      ...
      <div id="products-${order.orderId}" class="products-container" style="display: none;">
        ${productCards.join('')}
      </div>
    </div>`;
}
...
async function markPaid(id){ 
  try {
     const response = await fetch('/admin/markPayed', {
         method: 'POST',
         headers: {
             'Content-Type': 'application/json',
         },
...
async function markReady(id){ 
  try {
     const response = await fetch('/admin/markReady', {
         method: 'POST',
         headers: {
             'Content-Type': 'application/json',
         },
